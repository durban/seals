...
private[seals] object SchemaMacros {
  ...
  def schemaImpl(c: BContext)(annottees: c.Expr[Any]*): c.Expr[Any] = {
    ...
    def transformAnns(anns: List[c.Tree]): List[c.Tree] = {
      anns.filter {
        case pq"_root_.dev.tauri.seals.schema" => false
        case _ => true
      } :+ q"new _root_.dev.tauri.seals.schemaMarker()"
    }
    ...
    def injectedDefis(cls: TypeName): List[c.Tree] = {
      ...
      List(
        q"""
          implicit final lazy val ${valName}: _root_.dev.tauri.seals.core.Reified[${cls}] =
            _root_.shapeless.cachedImplicit[_root_.dev.tauri.seals.core.Reified[${cls}]]
        """
      )
    }
